    ...
    return (
        <div className="max-w-6xl mx-auto w-full px-4">
            <div className="text-center mb-16">
                <h1 className="text-4xl font-bold mb-2 uppercase tracking-widest">Programming Skills</h1>
                <p className="text-base-content/70 tracking-widest uppercase">List of My Skill Set</p>
            </div>

            <h4 className="text-center text-sm mb-4 text-base-content/70 tracking-widest">
                <span className="font-bold text-primary">{skillCount}</span>
                {showGroup === "all" ? (
                    <> skills across <span className="font-bold text-primary">{skillCategories.length}</span> categories</>
                ) : (
                ...
                <div className="flex justify-center">
                    <div className="join bg-base-200/40 backdrop-blur-sm border border-base-content/5 shadow-sm p-1 rounded-lg">
                        {skillGroups.map((group) => (
                            <button
                                key={group.value}
                                className={`btn btn-md join-item transition-all ${showGroup === group.value ? "btn-primary" : "btn-ghost opacity-60"
                                    }`}
                            ...
                            <div key={category.id} className="flex justify-center">
                                <div className="card w-full min-w-[300px] bg-base-200/40 backdrop-blur-sm border border-base-content/5 shadow-sm">
                                    <div className="card-body p-8 lg:px-8 lg:py-4">
                                        <h4 className="text-lg text-center font-bold tracking-widest uppercase mb-2">
                                            {category.title}
                                        </h4>
